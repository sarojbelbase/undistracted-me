...
vi.mock("../../../src/store", () => ({
  useSettingsStore: vi.fn((selector) => {
    const state = {
      ...
      language: "en",
      cardStyle: "glass",
      fmAudio: { volume: 0.7, binauralPreset: "focus" },
      setFmAudio: vi.fn(),
      focusTasks: true,
    };
...
vi.mock("../../../src/components/FocusMode/WorldClocksPanel", () => ({
  WorldClocksPanel: ({ timezones }) => (
    <div data-testid="world-clocks">{timezones?.join(",")}</div>
  ),
}));
...
// Audio engine — no-op in jsdom (AudioContext not available)
vi.mock("../../../src/components/FocusMode/audio/engine", () => ({
  audioEngine: {
    unlock: vi.fn(),
    start: vi.fn(),
    stop: vi.fn(),
...
vi.mock("../../../src/utilities/index", () => ({
  ...
  getGregorianDateParts: vi.fn(() => ({
    dow: "Sunday",
    month: "June",
    day: 15,
    year: 2025,
  })),
  getBikramSambatDateParts: vi.fn(() => ({
    dow: "Sunday",
    month: "Ashadh",
    day: 1,
    year: 2082,
...
import {
  ...
  useFocusStocks,
  useFocusPhoto,
} from "../../../src/components/FocusMode/hooks";
...
describe("FocusMode", () => {
  ...
  it("renders LeftPanel when pomodoro is active", () => {
    vi.mocked(readPomodoro).mockReturnValue({
      label: "Focus",
      remaining: 1500,
    });
    render(<FocusMode onExit={onExit} />);
  ...
  it("renders LeftPanel when events are present", () => {
    vi.mocked(getNextEventToShow).mockReturnValue({
      title: "Meeting",
      startTime: "10:00",
    });
    render(<FocusMode onExit={onExit} />);
