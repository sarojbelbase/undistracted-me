...
describe("parseWeather", () => {
  ...
  it("returns correct description", () => {
    expect(
      parseWeather(mockOpenMeteoResponse, "Kathmandu").description,
    ).toBeTruthy();
  });
  ...
  it("returns city name", () => {
    expect(parseWeather(mockOpenMeteoResponse, "Kathmandu").city).toBe(
      "Kathmandu",
    );
  });
  ...
  it("isDay = false when is_day = 0", () => {
    const night = {
      ...
      current: { ...mockOpenMeteoResponse.current, is_day: 0 },
    };
  ...
  it("isDay = true when is_day = 1 (duplicate for clarity)", () => {
    const day = {
      ...mockOpenMeteoResponse,
      current: { ...mockOpenMeteoResponse.current, is_day: 1 },
    };
  ...
  it("resolves with { lat, lon } when geolocation succeeds", async () => {
    const mockGeolocation = {
      getCurrentPosition: vi.fn((success) =>
        success({ coords: { latitude: 27.7172, longitude: 85.324 } }),
      ),
    };
